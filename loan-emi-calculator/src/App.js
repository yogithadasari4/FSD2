          ...
          {totalPayment !== 0 && monthlyPayment !== 0 && (
            <div className="mt-4">
              ...
              <Col xs={6} className="text-left">
                <h6 style={{ color: 'darkblue' }}>Rs. {(+totalPayment).toLocaleString('en-IN', { maximumFractionDigits: 2 })} /-</h6>
                <h6 style={{ color: 'darkblue' }}>Rs. {(+principal).toLocaleString('en-IN', { maximumFractionDigits: 2 })} /-</h6>
                <h6 style={{ color: 'darkblue' }}>Rs. {((+totalPayment) - (+principal)).toLocaleString('en-IN', { maximumFractionDigits: 2 })} /-</h6>
                <h6 style={{ color: 'red' }}>Rs. {(+monthlyPayment).toLocaleString('en-IN', { maximumFractionDigits: 2 })} /- <br></br>(per Month)</h6>
              </Col>
              </Row>            
            </div>
          )}
  ...
  {emi.length > 0 && (
        ...
        <tbody>
          {emi.map((entry) => (
            <tr key={entry.month}>
              <td>{entry.month}</td>
              <td>Rs. {Math.floor(entry.principal)}</td>
              <td>Rs. {Math.floor(entry.interest)}</td>
              <td>Rs. {Math.floor(entry.balance)}</td>
            </tr>
          ))}
        </tbody>
        </Table>
